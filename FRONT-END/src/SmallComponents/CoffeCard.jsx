  ...
  return (
    <div >
        ...
        <Box padding="30px" bg="white" borderTopRightRadius={"25px"} w="80%" borderBottomLeftRadius={"25px"} overflow={"hidden"} >
            ...
            <Box ml="5px">
                {/* name */}
            <Text mt="5px" fontWeight={"bold"} fontSize={"24px"} paddingRight={"100px"}  noOfLines={1}>{name}</Text> 
            
            {/* price */}
            <Text mt="5px" fontSize={"18px"}>$ {price}</Text>

            

                <Link to={`/coffee/${id}`}>
                    <Box mt="7px" display={"flex"} alignItems={"center"} gap="10px" fontSize={"20px"}>
